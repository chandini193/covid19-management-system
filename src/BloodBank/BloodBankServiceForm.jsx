...
class BloodBankServiceForm extends Component {
  constructor(props) {
    super(props)
    this.state = {
      bloodBankName: '',
  ...
  handleSubmit = (e) => {
    ...
    if (this.validateForm()) {
      axois.post("http://localhost:5000/BloodBank", this.state).then(alert('Form submitted'))
        .then(response => {
  ...
  validateForm() {
    let bloodBankName = this.state.bloodBankName;
    ...
    if (!bloodBankName) {
      formIsValid = false;
      nameError = "*Please enter BloodBank Name.";
    }

    if (typeof bloodBankName !== "undefined") {
      if (!bloodBankName.match(/^[a-zA-Z ]*$/)) {
        formIsValid = false;
  ...
  render() {
    const { bloodBankName, email, phoneNumber, address, city, state_name, district_name, remarks } = this.state
    return (
      <div className="App">
        <form onSubmit={this.handleSubmit} >
          <div className="information">
            <h3>Blood Bank Service Form</h3><br />
            <label> <FiUser />{" "}Blood Bank Name:</label>
            <input type="text" name="bloodBankName" value={bloodBankName} onChange={this.handleChange} />
...
export default BloodBankServiceForm;